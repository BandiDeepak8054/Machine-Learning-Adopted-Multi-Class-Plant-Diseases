...
def Proposed_Classifier():
    ...
    calculateMetrics("Proposed KNN", y_test, y_pred)
...
def Proposed_Classifier1():
    ...
    y_pred = classifier.predict(X_test)
    calculateMetrics("Proposed DecisionTree", y_test, y_pred)
...
def connect_db():
    return pymysql.connect(host='localhost', user='root', password='admin', database='sparse_db')
...
main.title("Machine Learning Adopted Multi-class Classification of Plant Diseases with Sparse and Categorical  IoT Data")

# Title
font = ('times', 18, 'bold')
title = tk.Label(main, text="Machine Learning Adopted Multi-class Classification of Plant Diseases with Sparse and Categorical  IoT Data", bg='white', fg='black', font=font, height=2, width=100)
title.pack()
